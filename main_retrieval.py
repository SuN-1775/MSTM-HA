...
def get_args(description='Disentangled Representation Learning for Text-Video Retrieval'):
    ...
    parser.add_argument('--lr', type=float, default=1e-3, help='learning rate for non-CLIP modules.')
    parser.add_argument('--coef_lr', type=float, default=1e-4,
                        help='CLIP learning-rate multiplier; lr * coef_lr = 1e-7 by default.')
    ...
    parser.add_argument('--agg_module', type=str, default="MSTE", choices=["None", "seqLSTM", "seqTransf", "MSTE"],
    ...
    parser.add_argument('--ssr_delta', type=float, default=0.75, help='SSR target similarity delta.')
    ...
    parser.add_argument("--query_number", default=8, type=int)
    ...
    # When using --use_env with torch.distributed.launch, LOCAL_RANK is set as env variable
    # not as command line argument, so we need to read it from environment
...
def prep_optimizer(args, model, num_train_optimization_steps, local_rank):
    ...
    logger.info(f"[Rank {local_rank}] Preparing optimizer, model has {num_params} parameters")
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        # In DistributedDataParallel mode, each process needs to move data to its own GPU
        # DDP does NOT automatically scatter data like DataParallel does
        ...
        if global_step % (log_step * 3) == 0 or global_step == 1:
            ...
            if args.local_rank == 0:
                # 只保存最佳模型，不保存中间步骤checkpoint以节省磁盘空间
...
def main():
    ...
    # Ensure all processes reach here before building model
    if torch.cuda.is_available():
        torch.distributed.barrier()

    model = build_model(args)
    
    # Synchronize after model initialization to ensure all ranks have loaded the model
    if torch.cuda.is_available():
        torch.distributed.barrier()
        logger.info(f"Model built successfully on rank {args.local_rank}, total params: {sum(p.numel() for p in model.parameters())}")
    ...
    if args.do_train:
        ...
        # Synchronize before wrapping model with DDP
        if torch.cuda.is_available():
            torch.distributed.barrier()
        ...
        for epoch in range(args.epochs):
            ...
            if args.local_rank == 0:
                ...
                if best_score <= R1:
                    ...
                    best_pth_path = join(args.output_dir, 'best.pth')
        ...
        if args.local_rank == 0:
            ...
            if os.path.exists(best_pth_path):
                model.load_state_dict(torch.load(best_pth_path, map_location='cpu'), strict=False)
                logger.info("Loaded best model from: {}".format(best_pth_path))
            else:
                logger.warning("Best model file not found: {}, using current model state".format(best_pth_path))
        if torch.cuda.is_available():
